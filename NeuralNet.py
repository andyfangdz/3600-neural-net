...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """
        newWeights = [alpha * delta * x + w for w, x in zip(self.weights, [1] + inActs)]
        totalModification = sum(abs(nW - w) for w, nW in zip(self.weights, newWeights))
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            output.append([p.sigmoidActivation(output[-1]) for p in layer])
        return output
        ...
        for example in examples:  # for each example
            ...
            for layerNum in xrange(self.numHiddenLayers - 1, -1, -1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    delta = gPrime * sum(p.weights[neuronNum + 1] * d for p, d in zip(nextLayer, deltas[0]))
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold=0.00008, hiddenLayerList=[1], maxItr=sys.maxint,
                   startNNet=None):
    """
    ...
    print >>stderr, "Starting training at time %s with %d inputs, %d outputs, %s hidden layers, size of training set %d, and size of test set %d" \
    ...
    time = datetime.now().time()
    print >>stderr, 'Finished after %d iterations at time %s with training error %f and weight change %f' % (
    ...
    for features, label in examplesTest:
        outputs = nnet.feedForward(features)
        if all(round(a) == round(b) for a, b in zip(outputs[-1], label)):
            testCorrect += 1
        else:
            testError += 1
    ...
    print >>stderr, 'Feed Forward Test correctly classified %d, incorrectly classified %d, test percent error  %f\n' % (
